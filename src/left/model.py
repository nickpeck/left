...
class LeftModel:
    ...
    async def upsert(self):
        ...
        if self.key is None:
            self.key = str(uuid4())
            await self.get_resource_for_cls().create(**self.to_dict())
            return self
        payload = self.to_dict()
        await self.get_resource_for_cls().update(key_value=self.key, **payload)
        return self

    @classmethod
    async def get(cls, key: str) -> LeftModel:
        """Return the first object with the matching key"""
        key_query = {cls.__pk__: key}
        record = await cls.get_resource_for_cls().read(**key_query)[0]
        return cls.from_dict(record)

    @classmethod
    async def all(cls) -> List[LeftModel]:
        """Return all records of this type"""
        records = await cls.get_resource_for_cls().read()
        return [cls.from_dict(record) for record in records]

    @classmethod
    async def get_where(cls, **kwargs) -> List[LeftModel]:
        """Return a list of all records of this type with matching attributes as specified, chained in AND syntax"""
        records = await cls.get_resource_for_cls().read(**kwargs)
        return [cls.from_dict(record) for record in records]

    @classmethod
    async def search(cls, **kwargs) -> List[LeftModel]:
        """Return a list of all records of this type with matching attributes as specified, chained in OR syntax"""
        records = await cls.get_resource_for_cls().read(operator="or", **kwargs)
        return [cls.from_dict(record) for record in records]

    async def delete(self):
        """Delete the record with the matching key from the database"""
        await self.get_resource_for_cls().destroy(key_value=self.key)
